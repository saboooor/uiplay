...
pub async fn kill_uxplay(app: tauri::AppHandle) {
    ...
    match check {
        ...
        Ok(_) => {
            log_output(
                app.clone(),
                "UxPlay is not running, starting a new instance...",
            );
        }
        Err(e) => {
            log_output(
                app.clone(),
                format!("Failed to check if UxPlay is running: {}", e),
            );
            return;
        }
    }
...
pub async fn start_uxplay(app: tauri::AppHandle) {
    ...
    if let Err(e) = client.connect() {
        log_output(
            app.clone(),
            format!("Failed to connect to Discord IPC: {:?}", e),
        );
    } else {
    ...
    log_output(
        app.clone(),
        format!("UxPlay process exited with status: {}", status),
    );
